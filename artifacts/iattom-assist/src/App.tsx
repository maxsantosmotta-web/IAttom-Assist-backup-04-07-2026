...
function PageLoader() {
  return <div className="flex items-center justify-center min-h-[40vh]"><div className="w-9 h-9 rounded-full border-2 border-primary/20 border-t-primary animate-spin" /></div>;
}
...
const clerkAppearance = {
  ...
  variables: {
    colorPrimary: "#C9A84C", colorForeground: "#fafafa", colorMutedForeground: "#71717a", colorDanger: "#ef4444",
    colorBackground: "#111111", colorInput: "#0a0a0a", colorInputForeground: "#fafafa", colorNeutral: "#3f3f46",
    fontFamily: "Inter, sans-serif", borderRadius: "0.5rem",
  },
  elements: {
    rootBox: "w-full flex justify-center", cardBox: "bg-[#111111] border border-white/10 rounded-xl w-[440px] max-w-full overflow-hidden shadow-2xl",
    card: "!shadow-none !border-0 !bg-transparent !rounded-none", footer: "!shadow-none !border-0 !bg-transparent !rounded-none",
    headerTitle: "text-white font-bold", headerSubtitle: "text-zinc-400", socialButtonsBlockButtonText: "text-white",
    formFieldLabel: "text-zinc-300 text-sm", footerActionLink: "text-[#C9A84C] hover:text-[#E8C96A]", footerActionText: "text-zinc-500",
    dividerText: "text-zinc-600", identityPreviewEditButton: "text-[#C9A84C]", formFieldSuccessText: "text-emerald-400",
    alertText: "text-white", logoBox: "flex justify-center py-2", logoImage: "h-10 w-10",
    ...
    formFieldInput: "bg-[#0a0a0a] border-white/10 text-white", footerAction: "bg-[#0d0d0d] border-t border-white/5",
    dividerLine: "bg-white/10", alert: "bg-red-950/30 border border-red-500/20", otpCodeFieldInput: "bg-[#0a0a0a] border-white/10 text-white",
    formFieldRow: "gap-2", main: "gap-4",
  },
};
...
function SignUpCallbackPage() {
  const [location] = useLocation();
  return location.includes("/sso-callback") ? <AuthenticateWithRedirectCallback /> : <SignUpPage />;
}
...
function ProtectedOnboarding() {
  return <><Show when="signed-in"><Redirect to="/dashboard/billing" /></Show><Show when="signed-out"><Redirect to="/sign-up" /></Show></>;
}
...
function ProtectedAdmin() {
  return <>
    <Show when="signed-in"><AdminGuard><AdminLayout><Suspense fallback={<PageLoader />}><Switch>
      ...
      <Route component={NotFound} />
    </Switch></Suspense></AdminLayout></AdminGuard></Show>
    <Show when="signed-out"><Redirect to="/" /></Show>
  </>;
}
...
const clerkLocalization = {
  ...ptBR,
  unstable__errors: {
    ...(ptBR.unstable__errors ?? {}), not_allowed_access: BLOCKED_MSG, banned_user: BLOCKED_MSG, user_locked: BLOCKED_MSG,
    user_banned: BLOCKED_MSG, form_identifier_exists: "E-mail já cadastrado. Utilize Fazer Login.",
    form_identifier_not_found: "Usuário não encontrado. Crie sua conta.", form_password_incorrect: "E-mail ou senha inválidos.",
...
function BannedUserGuard() {
  ...
  useEffect(() => addListener(({ session }) => {
    if (session && (session as { status?: string }).status === "revoked") setBlocked(true);
  }), [addListener]);
  if (!blocked) return null;
  return <div role="alert" className="fixed inset-0 z-[9999] bg-[#0a0a0a] flex flex-col items-center justify-center gap-6 px-4">
    ...
    <button onClick={() => { setBlocked(false); window.location.href = basePath || "/"; }} className="text-xs text-zinc-500 underline">Voltar ao início</button>
  </div>;
}
...
function App() {
  ...
  return <WouterRouter base={basePath}>
    <AnimatePresence mode="wait" onExitComplete={() => setShowContent(true)}>{isLoading && <LoadingScreen key="loading" />}</AnimatePresence>
    <div style={!showContent ? { visibility: "hidden", position: "fixed", inset: 0, overflow: "hidden", pointerEvents: "none" } : undefined} aria-hidden={!showContent}>
