  ...
  const generatePromptCore = async () => {
    ...
    try {
      ...
      if (res.ok && data.title && data.prompt) {
        ...
        setGenerated(true);
        toast({ description: "Prompt gerado." });
      } else {
        ...
        toast({ description: data.error ?? "Erro ao gerar prompt.", variant: "destructive" });
      }
    } catch {
      if (chargedRef.current) {
        void fetch("/api/credits/refund", {
  ...
  return (
    <div className="space-y-6 pb-4">
      <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
        <p className="text-[10px] text-primary font-bold tracking-widest uppercase">Criação</p>
        <h2 className="text-2xl font-black tracking-tight text-white">Criar Prompt</h2>
        <p className="text-sm text-zinc-500">Crie prompts profissionais e salve tudo na Biblioteca.</p>
      </motion.div>
      ...
      <div className="bg-[#0f0f0f] border border-primary/20 rounded-2xl p-5 space-y-4">
        <div className="space-y-2">
          ...
          <div className="flex flex-wrap gap-1.5">
            {TIPO_OPTIONS.map((tipo) => (
              <button
                key={tipo}
                onClick={() => setGuidedTipo(tipo)}
                className={`text-[11px] font-semibold px-3 py-1.5 rounded-lg border transition-all duration-150 ${guidedTipo === tipo ? "bg-primary/20 text-primary border-primary/40" : "text-zinc-500 border-white/[0.07] hover:text-zinc-300 hover:border-white/15"}`}
              >
                {tipo}
              </button>
            ))}
        ...
        >
          {({ trigger, isLoading }) => (
            <Button onClick={trigger} disabled={!canGenerate || generating || isLoading} className="bg-primary text-black hover:bg-primary/90 w-full font-bold">
        ...
        <AnimatePresence>
          {generated && (
            <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="space-y-3 pt-3 border-t border-white/[0.06]">
              <div className="space-y-1.5">
              ...
              <div className="space-y-1.5">
                ...
                <Textarea value={newPrompt} onChange={(event) => setNewPrompt(event.target.value)} className="bg-[#111111] border-white/[0.08] text-zinc-200 min-h-40 resize-none text-xs leading-relaxed" />
              </div>
