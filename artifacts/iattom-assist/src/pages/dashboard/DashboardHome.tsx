...
const quickActions: QuickAction[] = [
  ...
  { href: "/dashboard/creative-generator", label: "Vídeo com Efeito", icon: Video, desc: "Dê movimento às suas imagens", color: "text-cyan-400", bg: "bg-cyan-400/10 border-cyan-400/20", glow: "hover:shadow-[0_0_30px_-6px_rgba(34,211,238,0.15)]", module: "video_effect", creativeMode: "video" },
  { href: "/dashboard/creative-generator", label: "Gerar Imagem", icon: Sparkles, desc: "Crie imagens profissionais", color: "text-purple-400", bg: "bg-purple-400/10 border-purple-400/20", glow: "hover:shadow-[0_0_30px_-6px_rgba(192,132,252,0.15)]", module: "creative", creativeMode: "image" },
...
const MODULE_ALIASES: Record<string, string> = {
  find_products: "product_discovery",
  validate_products: "product_validation",
  marketing: "campaign",
};
...
function selectCreativeMode(action: QuickAction): void {
  if (!action.creativeMode) return;
  try {
    localStorage.setItem("iattom_creative_tab_v1", action.creativeMode);
  } catch {
    // A navegação continua mesmo se o armazenamento local estiver indisponível.
  }
}
          ...
          >
            ...
            <div className="space-y-2">
              {recentModules.map((action) => {
                ...
                return (
                  <Link
                    key={`${action.module}:${action.creativeMode ?? "default"}`}
                    href={action.href}
                    onClick={() => selectCreativeMode(action)}
                  >
      ...
      <div>
        ...
        >
          {quickActions.map((action) => {
            const Icon = action.icon;
            return (
              <motion.div key={`${action.module}:${action.creativeMode ?? "default"}`} variants={itemVariants}>
                <Link
                  href={action.href}
                  onClick={() => selectCreativeMode(action)}
