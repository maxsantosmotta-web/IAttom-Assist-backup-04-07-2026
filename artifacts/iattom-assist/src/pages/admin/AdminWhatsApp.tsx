...
import {
  MessageCircle, RefreshCw, Loader2, Users, Activity, Clock, BarChart2,
} from "lucide-react";
...
export function AdminWhatsApp() {
  ...
  const loadEvents = useCallback(async () => {
    ...
    try { setEvents(await apiFetch<WhatsAppEventItem[]>("/api/whatsapp/events")); }
    catch { setEvents([]); }
  ...
  const kpis = [
    { label: "Usuários Conectados", value: "—", icon: Users,    color: "text-green-400"   },
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-4 justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-zinc-500">Monitoramento de conexões dos usuários com o WhatsApp Business.</p>
            </div>
          </div>
      ...
      <Card className="bg-white/3 border-white/8">
        ...
        <CardContent>
          <div className="flex flex-col items-center justify-center py-8 text-center gap-2">
            ...
            <p className="text-sm text-zinc-500">Nenhum usuário conectado ao WhatsApp Business.</p>
        ...
        <CardContent>
          {loadingEvents ? (
            ...
            <p className="text-xs text-zinc-600 text-center py-6">
              Nenhuma mensagem registrada. Os webhooks do WhatsApp aparecerão aqui.
            </p>
          ) : (
              ...
              {events.slice(0, 50).map(ev => (
                <div key={ev.id} className="py-2.5 flex items-start gap-3">
                  <div className="flex-1 min-w-0">
                    ...
                    {ev.fromNumber && <p className="text-[10px] text-zinc-500 font-mono">{ev.fromNumber}</p>}
                  </div>
