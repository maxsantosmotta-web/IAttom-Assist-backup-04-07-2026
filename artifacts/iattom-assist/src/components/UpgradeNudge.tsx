...
export function UpgradeNudge({ totalActions = 0 }: UpgradeNudgeProps) {
  ...
  const configs = {
    ...
    low: {
      ...
      body: `Você tem ${creditsLeft} créditos este mês. Atualize para ${PLAN_CREDITS.pro.toLocaleString()}+ créditos e acesso prioritário.`,
      cta: "Ver planos",
      accentCta: false,
    },
    power: {
      ...
      body: `${totalActions} execuções e contando. O plano Pro oferece ${PLAN_CREDITS.pro.toLocaleString()} créditos/mês — muito mais potência.`,
      cta: "Desbloquear Pro",
      accentCta: true,
    },
  };
