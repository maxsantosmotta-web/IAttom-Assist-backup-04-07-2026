...
function insertActions(relativePath, functionName, withRefresh) {
  ...
  const fnIndex = original.indexOf(`export function ${functionName}`);
  if (fnIndex < 0) return;
  const returnIndex = original.indexOf("return (", fnIndex);
  if (returnIndex < 0) return;
  const divIndex = original.indexOf("<div", returnIndex);
  if (divIndex < 0) return;
  const closeIndex = original.indexOf(">", divIndex);
  if (closeIndex < 0) return;

  const source = original.slice(0, closeIndex + 1) + actionBar(withRefresh) + original.slice(closeIndex + 1);
  write(url, source);
}

function makeExistingRefreshReload(relativePath, functionName, emphasize = false) {
  const { url, source: original } = read(relativePath);
  const fnIndex = original.indexOf(`export function ${functionName}`);
  if (fnIndex < 0) return false;
  const updateIndex = original.indexOf("Atualizar", fnIndex);
  if (updateIndex < 0) return false;

  const buttonStart = Math.max(original.lastIndexOf("<Button", updateIndex), original.lastIndexOf("<button", updateIndex));
  if (buttonStart < 0) return false;
  const openingEnd = original.indexOf(">", buttonStart);
  if (openingEnd < 0 || openingEnd > updateIndex) return false;

  let opening = original.slice(buttonStart, openingEnd + 1);
  if (/onClick=\{[^>]*\}/s.test(opening)) {
    opening = opening.replace(/onClick=\{[^>]*\}/s, "onClick={() => window.location.reload()}");
  } else {
    opening = opening.replace(/^(<Button|<button)/, '$1 onClick={() => window.location.reload()}');
  }

  if (emphasize) {
    opening = opening.replace(/className="[^"]*"/, 'className="inline-flex h-9 items-center justify-center rounded-md border border-primary/40 bg-primary/15 px-3 text-sm font-semibold text-primary transition-colors hover:bg-primary/25"');
  }

  const source = original.slice(0, buttonStart) + opening + original.slice(openingEnd + 1);
  write(url, source);
  return true;
}
...
for (const [path, name] of [
  ...
  insertActions(path, name, !hasRefresh);
}

const dashboardHasRefresh = makeExistingRefreshReload("pages/dashboard/DashboardHome.tsx", "DashboardHome", true);
insertActions("pages/dashboard/DashboardHome.tsx", "DashboardHome", !dashboardHasRefresh);
...
console.log("Navigation and library recovery applied without removing existing JSX blocks.");
