...
if (!source.includes("annualSubscriptions: {\n    total: number;")) {
  if (!source.includes(summaryTypeAnchor)) throw new Error("Finance annual UI type anchor not found");
...
`;
if (!source.includes(">Planos Anuais</h3>")) {
  if (!source.includes(chartsAnchor)) throw new Error("Finance annual UI insertion anchor not found");
...
for (const marker of [
  ...
  if (!source.includes(marker)) throw new Error(`Finance annual UI marker missing: ${marker}`);
}
...
for (const forbidden of [
  ...
  if (source.includes(forbidden)) throw new Error(`Visible Finance diagnostic detected: ${forbidden}`);
}

fs.writeFileSync(financePath, source);
console.log("Admin Finance annual block added without visible diagnostics or parallel data sources.");
