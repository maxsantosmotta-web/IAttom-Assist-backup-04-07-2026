...
function replaceOnce(before, after, label) {
  ...
  if (!source.includes(before)) throw new Error(`Admin users UI v2 anchor not found: ${label}`);
...
String.raw`  const openCreditAdjust = (user: AdminUser) => {
...
if (oldHandlerStart < 0 || oldHandlerEnd < 0) throw new Error("Admin users UI v2 anchor not found: balance handler");
const newHandler = String.raw`  const handleCreditAdjust = () => {
...
if (dialogStart < 0 || dialogEnd < 0) throw new Error("Admin users UI v2 anchor not found: balance dialog");
...
console.log("Admin users public plan names and complete balance controls patched safely.");
