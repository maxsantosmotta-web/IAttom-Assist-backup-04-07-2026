...
if (source.includes(legacyRunGenerate)) {
  source = source.replace(legacyRunGenerate, directChargeRunGenerate);
} else if (source.includes(serverOnlyRunGenerate)) {
  source = source.replace(serverOnlyRunGenerate, directChargeRunGenerate);
}
...
if (!source.includes('fetch("/api/credits/use"') || !source.includes("await refetchCredits();")) {
  throw new Error("Creative credit integrity patch did not install direct image debit");
}
...
console.log("Creative generator now debits exactly 1 or 2 images and refreshes the balance after success");