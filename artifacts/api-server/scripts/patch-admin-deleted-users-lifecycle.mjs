...
const helperMarker = "async function ensureDeletedUsersAuditTable()";
if (!admin.includes(helperMarker)) {
...
const ACTIVE_USER_CONDITION = sql\`\${users.email} NOT LIKE \${`%${DELETED_EMAIL_SUFFIX}`}\`;
...
router.delete("/admin/users/:id/remove-manual", requireAdmin, async (req, res): Promise<void> => {
  ...
  await db.execute(sql\`
    INSERT INTO deleted_users_audit (
      original_email, original_name, previous_plan, deleted_at, deleted_by, previous_clerk_id
    ) VALUES (
      \${targetUser.email}, \${targetUser.name}, \${targetUser.plan}, NOW(), \${adminIdentity}, \${targetUser.clerkId}
    )
...
if (!userSync.includes('DELETE FROM deleted_users_audit')) {
  ...
  userSync = userSync.replace(syncAnchor, `${syncAnchor}
  if (email) {
    await db.execute(sql\`
      CREATE TABLE IF NOT EXISTS deleted_users_audit (
        id BIGSERIAL PRIMARY KEY,
