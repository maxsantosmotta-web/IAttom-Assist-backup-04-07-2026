...
service = service
  ...
  .replace(/line_items: \[\{ price: priceId, quantity: 1 \}\],/g, (match, offset, full) => {
    const before = full.slice(Math.max(0, offset - 1200), offset);
    if (before.includes("createCreditPurchaseCheckoutSession")) {
      return `line_items: [
      {
        price_data: {
          currency: "brl",
    ...
    if (before.includes("createCreativePurchaseCheckoutSession")) {
      return `line_items: [
      {
        price_data: {
          currency: "brl",
...
console.log("Complete billing test runtime applied: plan limits, test prices, packages and session reconciliation aligned.");
