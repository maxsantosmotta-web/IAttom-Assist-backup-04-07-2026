...
if (historyStart === -1 || historyEnd === -1 || historyEnd <= historyStart) {
  throw new Error("Finance legacy movement block boundaries not found");
}
...
const chargeHistoryBlock = `    const successfulCharges = charges.filter((charge) =>
    ...
    const movements: FinancialMovement[] = successfulCharges.map((charge) => {
      ...
      return {
        ...
        userEmail: user?.email ?? charge.billing_details.email ?? "Cliente Stripe",
        plan: PLAN_NAMES[planKey] ?? planKey,
...
if (!source.includes("const chargeRevenueCents = successfulCharges.reduce")) {
  if (!source.includes(revenueAnchor)) throw new Error("Finance legacy revenue calculation anchor not found");
...
for (const forbidden of [
  ...
  if (source.includes(forbidden)) throw new Error(`Finance still depends on deletable customer history: ${forbidden}`);
}

fs.writeFileSync(growthPath, source);
console.log("Admin Finance history now uses successful Stripe charges independently of current customer records.");
